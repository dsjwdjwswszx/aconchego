...
const Cart = () => {
  const { state, removeItem, updateQuantity, calculateTotal, setDeliveryMethod } = useCart();
        ...
        {isAuthenticated ? (
          ...
          <div className="w-full space-y-2">
            ...
            <p className="text-sm text-muted-foreground text-center">
              É necessário ter uma conta para fazer pedidos
            </p>
          </div>
        )}
      </CardFooter>
...
const Badge = ({ item }) => {
  ...
  switch (item.category.toLowerCase()) {
    case 'furniture':
      color = "bg-purple-dark/30 text-purple-light";
      break;
    case 'decoration':
      color = "bg-green-800/30 text-green-400";
      break;
    case 'appliance':
